 ...
 * возвращаем false, но НЕ роняем основной сценарий (подписку/рассылку).
 *
 * Паттерн скопирован из andreev-site (src/lib/mailer.ts) — тот же relay
 * (Timeweb), но ADMIN_EMAIL на этом сервере должен указывать на
 * AndreevBank@bk.ru (контакт Егора-поэта), а не на адрес другого проекта.
 */
